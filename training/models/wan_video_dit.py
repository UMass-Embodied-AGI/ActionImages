...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs, grid_size):
        ...
        _, h, w = grid_size
        cam_emb = cam_emb.unsqueeze(2).unsqueeze(3).repeat(1, 1, h, w, 1)  # (b, 2, h, w, d)
        cam_emb = rearrange(cam_emb, "b f h w d -> b (f h w) d")  # (b, 2*h*w, d)
        # if their shape is different
        if cam_emb.shape != input_x.shape:
            cam_emb = chunk_for_usp(cam_emb)
        # letting all the processes have the same cam_emb, chunking locally, this is a bit slower.
